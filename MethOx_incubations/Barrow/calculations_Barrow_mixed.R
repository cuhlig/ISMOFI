...
#mixed concentration range Barrow
...
#calibration table for HP range Barrow
cali.HP.Barrow <- read.table("../Barrow.cali.HP.final.txt", header=TRUE, sep="\t", comment.char="", stringsAsFactors=FALSE)
cali.HR.Barrow <- read.table("../Barrow.cali.HR.final.txt", header=TRUE, sep="\t", comment.char="", stringsAsFactors=FALSE)
...
for (file_name in file_names) {
  ...
  data.oxi.HR <- subset(data.oxi, data.oxi$HP.Delta.iCH4.Mean > 1000)
  data.oxi.HR <- merge(data.oxi.HR, cali.HR.Barrow, by.x = "cali.date", by.y = "date", all.x = TRUE)

  data.oxi.HP <- subset(data.oxi, data.oxi$HP.Delta.iCH4.Mean < 1000)
  data.oxi.HP <- merge(data.oxi.HP, cali.HP.Barrow, by.x = "cali.date", by.y = "date", all.x = TRUE)
  
  #calculate calibrated CH4 concentration and delta HR range
  ...
  calibrated.CH4.HR <- calCH4deltappm(data.oxi.HR$HR.CH4.Mean, data.oxi.HR$b.ppm, data.oxi.HR$m.ppm, data.oxi.HR$SSIM.dil.factor, data.oxi.HR$HR.Delta.iCH4.Mean, data.oxi.HR$b.delta, data.oxi.HR$m.delta)

  data.oxi.HR <- cbind(data.oxi.HR, calibrated.CH4.HR)  
   
  
  #calculate calibrated CH4 concentration and delta HP range
  source("../../../R_lib/calibrateCH4deltappm.R") #  calCH4deltappm <- function(Picarro.ppm, b.ppm, m.ppm, data.oxi$SSIM.dil.factor, Picarro.delta, b.delta, m.delta){
  calibrated.CH4.HP <- calCH4deltappm(data.oxi.HP$HP.CH4.Mean, data.oxi.HP$b.ppm, data.oxi.HP$m.ppm, data.oxi.HP$SSIM.dil.factor, data.oxi.HP$HP.Delta.iCH4.Mean.corr, data.oxi.HP$b.delta, data.oxi.HP$m.delta)
  
  data.oxi.HP <- cbind(data.oxi.HP, calibrated.CH4.HP)  
  
  
  data.oxi <- rbind(data.oxi.HR, data.oxi.HP)
  
  #calculate time since start of experiment in days from timestamp
  source("../../../R_lib/daysfromts.R")
  data.oxi$time.days <- daysfromts(data.oxi$Timestamp.cor)
  ...
  # source("D:/GoogleDrive/rlib/nCH4sampled_new.R") #CH4sol
  # corrected.CH4.toal <-
  # nCH4sampled_new(
  #   data.oxi$vol.mL,
  #   data.oxi$V.discarded,
  #   data.oxi$V.hs.initial,
  #   data.oxi$hs.removed,
  #   data.oxi$Date.ID,
  #   data.oxi$n.CH4.hs,
  #   data.oxi$n.CH4.total
  # )
  # #print(corrected.CH4.toal)
  # data.oxi <- cbind(data.oxi, corrected.CH4.toal)
...
#subset data if some days shout not be used for fits
data.oxi.sel <- subset(data.oxi, time.days > 6)
