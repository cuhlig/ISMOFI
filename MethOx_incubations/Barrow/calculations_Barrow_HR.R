...
# 500x Peri; 5 bags 27, 34, 44; difficult to determine if days are bad; 30,38 in Mixed
# 500x Submers; 5 bags 25, 28, 32, 35, 36; difficult to determine if days are bad
...
for (file_name in file_names) {
  ...
  data.oxi <- merge(data.oxi, cali.HR.Barrow, by.x = "cali.date", by.y = "date", all.x = TRUE)
  ...
  calibrated.CH4.HR <- calCH4deltappm(data.oxi$HR.CH4.Mean, data.oxi$b.ppm, data.oxi$m.ppm, data.oxi$SSIM.dil.factor, data.oxi$HR.Delta.iCH4.Mean, data.oxi$b.delta, data.oxi$m.delta)

  #print(calibrated.CH4.HP)
  data.oxi <- cbind(data.oxi, calibrated.CH4.HR)  
...
data.oxi.sel <- subset(data.oxi, time.days > 3)
