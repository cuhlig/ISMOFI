...
#mixed concentration range JulAug
...
cali.HR.JulAug <- read.table("../JulAug.cali.HR.final.txt", header=TRUE, sep="\t", comment.char="", stringsAsFactors=FALSE)


# 500x Narragansett Bay; 2 bags 57, 60
...
for (file_name in file_names) {
  ...
  data.oxi.HR <- merge(data.oxi.HR, cali.HR.JulAug, by.x = "Date", by.y = "date", all.x = TRUE)

  data.oxi.HP <- subset(data.oxi, data.oxi$HP.Delta.iCH4.Mean < 1000)
  data.oxi.HP <- merge(data.oxi.HP, cali.HP.JulAug, by.x = "Date", by.y = "date", all.x = TRUE)
